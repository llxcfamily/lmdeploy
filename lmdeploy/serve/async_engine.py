...
class AsyncEngine:
    ...
    async def generate(
        ...
        logger.info(
            f"""traceid: {traceid}, prompt: {prompt}, instance_id: {instance_id}, 
            stream: {stream_response}, sequence_start:{sequence_start}, sequence_end:{sequence_end},
            request_output_len: {request_output_len},  top_k:{top_k}, top_p:{top_p}, 
            temperature:{temperature}, repetition_penalty:{repetition_penalty},
            step:{step}, ignore_eos:{ignore_eos}, stop:{stop}
            """
        )
        input_ids = self.tokenizer.encode(prompt)
        ...
        if self.steps[str(session_id)] + len(
            ...
            yield GenOut('', self.steps[str(session_id)], len(input_ids), 0, cost_time, finish_reason, True)
        else:
            ...
            with self.safe_run(instance_id):
                ...
                try:
                    async for outputs in generator.async_stream_infer(
                        ...
                        if finished == True:
                            cost_time = time.time() - start_time
                            logger.info(f"""traceid: {traceid}, text: {response}, input_tokens:{len(input_ids)}, 
                                generated_tokens: {tokens}, cost_time: {cost_time}, 
                                finish_reason: {finish_reason}, finished: {finished}
                                """
                            )
                            continue
                        # logger.info(f"res: {res}, tokens:{tokens}, eos: {self.tokenizer.eos_token_id}")
                        if res[-1] == self.tokenizer.eos_token_id:
                            finished = True
                            # decode res
                            response = self.tokenizer.decode(res.tolist())
                            delta = self.tokenizer.decode(res.tolist(),
                            ...
                            response = self.tokenizer.decode(res.tolist(),
                                                            offset=response_size)
                            delta = response
                            if len(delta) > 0 and not delta.endswith("�"):
                                yield_return = True
                            else:
                                yield_return = False
                            #logger.info(f"res: {response}")
                        ...
                        if yield_return:
                            if len(input_ids) + tokens >= self.tm_model.session_len:
                                finish_reason = 'length'
                                yield GenOut(delta, self.steps[str(session_id)], len(input_ids), tokens, 
                                        time.time() - start_time, finish_reason, True)
                            else:
                                yield GenOut(delta, self.steps[str(session_id)], len(input_ids), tokens, 
                                        time.time() - start_time, finish_reason, finished)
                    ...
                    logger.info(f"exception occured in async engine with error: {e}")
