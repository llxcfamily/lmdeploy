...
async def generate(request: GenerateRequest, raw_request: Request = None):
    ...
    generation = VariableInterface.async_engine.generate(
        ...
        sequence_start=request.data.get('sequence_start', True),
        sequence_end=request.data.get('sequence_end', True),
        request_output_len=request.data.get('output_len', 512),
        top_p=request.data.get('top_p', 0.85),
        top_k=request.data.get('top_k', 5),
        stop=request.data.get('stop', False),
        temperature=request.data.get('temperature', 0.7),
        repetition_penalty=request.data.get('repetition_penalty', 1.1),
        ignore_eos=request.data.get('ignore_eos', False),
        traceid=request.traceid,
        seed=request.data.get('random_seed', 42),
    )
    # Streaming case
    ...
    #async def stream_results() -> AsyncGenerator[bytes, None]:
        async for out in generation:
            ...
            yield ServerSentEvent(json.dumps(ret, ensure_ascii=False))
